	...
	if(0<$('.view-'+path[1]).length){
		...
		if(typeof path[2] != 'undefined'){
			if(0<$('.view-'+path[1]+' .page-'+path[2]).length){
				...
				if('fixed-award'==path[2]){
					clearTimeout(update_dgp_timer);
					update_dgp_timer=setTimeout(function(){
						update_dgp(true);
						$('.page-fixed-award input[name=fixed-award-account]').val('');
						$('.page-fixed-award input[name=fixed-award-amount]').val('');
						$('.page-fixed-award input[name=fixed-award-memo]').val('');
						$('.page-fixed-award input[name=fixed-award-account]').focus();
						if(typeof params.to !== "undefined"){
							$('.page-fixed-award input[name=fixed-award-account]').val(params.to);
						}
						if(typeof params.amount !== "undefined"){
							$('.page-fixed-award input[name=fixed-award-amount]').val(params.amount);
						}
						if(typeof params.memo !== "undefined"){
							$('.page-fixed-award input[name=fixed-award-memo]').val(params.memo);
						}
					},100);
					load_history($('.page-fixed-award .history'),false,true);
				}
				if('checks'==path[2]){
					/*
...
// Owner wants "markets about to end" surfaced by default — the node has no global expiration sort,
// so we pull newest (which are the freshly-seeded near-term markets, WITH metadata) and sort client-side.
function pm_close_key(m){ let be=pm_close_ts(m); let now=parseInt(Date.now()/1000); if(be===0){ return 9e15; } if(be<=now){ return 8e15+be; } return be; }
// Relative "closes in" — flags soon-ending markets so the card reads as urgent, not a far dead date.
function pm_reltime(be){
	let now=parseInt(Date.now()/1000), d=be-now;
	if(be===0){ return {txt:(ltmp_arr.pm_open_ended||'open-ended'),soon:false}; }
	if(d<=0){ return {txt:(ltmp_arr.pm_closed_lc||'closed'),soon:false}; }
	if(d<3600){ return {txt:(ltmp_arr.pm_soon||'soon'),soon:true}; }
	if(d<86400){ return {txt:ltmp(ltmp_arr.pm_in_hours||'in {n}h',{n:Math.round(d/3600)}),soon:true}; }
	return {txt:ltmp(ltmp_arr.pm_in_days||'in {n}d',{n:Math.round(d/86400)}),soon:(d<3*86400)};
}
// Status word colour: active=green, awaiting/closed/resolved=grey (reuse existing .green/.grey text classes).
function pm_status_class(m){ if(1==m.status){ let be=pm_close_ts(m); if(be>0 && be<parseInt(Date.now()/1000)){ return 'grey'; } return 'green'; } return 'grey'; }
function pm_market_card_html(m){
	let title=(''+(m.title||(m.metadata&&m.metadata.title)||'')).trim();
	let card='<div class="columns-view"><div class="column column-1 shadow grid pm-market-card">';
	if(m.image){ card+='<div class="center"><img style="max-height:70px" src="'+escape_html(''+m.image)+'" onerror="this.style.display=\'none\'"></div>'; }
	card+='<div class="bold">'+(title!==''?escape_html(title):('#'+m.id))+'</div>';
	card+='<div class="text-small grey">'+escape_html(''+(m.category||''))+' &middot; <span class="'+pm_status_class(m)+'">'+pm_market_status_label(m)+'</span></div>';
	// current outcome split — binary: implied probability from CPMM reserves; multi: number of options
	if(0==parseInt(m.market_type)){
		...
		card+='<div style="height:6px;border-radius:3px;overflow:hidden;background:#d94c4c;margin:5px 0"><div style="height:6px;width:'+pY+'%;background:#3a9d3a"></div></div>';
		card+='<div class="text-small"><b>'+pY+'%</b> '+(ltmp_arr.pm_yes||'Yes')+' &middot; <b>'+pN+'%</b> '+(ltmp_arr.pm_no||'No')+'</div>';
	}
	else{
		card+='<div class="text-small">'+ltmp((ltmp_arr.pm_options_n||'{n} options'),{n:parseInt(m.outcome_count)||0})+'</div>';
	}
	card+='<div class="text-small grey">'+(ltmp_arr.pm_volume||'Betting volume')+': '+pm_fmt_viz(m.bets_sum)+'</div>';
	let rt=pm_reltime(pm_close_ts(m));
	card+='<div class="text-small">'+(ltmp_arr.pm_closes||'Closes')+': <b class="'+(rt.soon?'green':'grey')+'">'+escape_html(rt.txt)+'</b> <span class="grey">('+show_date(m.betting_expiration,true)+ltmp_arr.default_date_utc+')</span></div>';
	card+='<div class="wide-buttons captions"><a class="wide-button" data-href="/pm/market/'+m.id+'/">'+(ltmp_arr.pm_open||'Open')+'</a>';
	if(m.url){ card+='<a class="wide-button" href="'+escape_html(''+m.url)+'" target="_blank">'+(ltmp_arr.pm_source||'Source')+' &#8599;</a>'; }
	card+='</div></div></div>';
	return card;
}
// Render the accumulated markets, sorted soonest-closing first, applying the text filter.
...
function load_pm_markets(reset){
	...
	if(reset){ pm_markets_page=0; pm_markets_all=[]; list.html(''); }
...
function load_pm_positions(market_id,m,ocs){
	...
	viz.api.getAccountPositions(current_user,0,200,function(err,list){
		...
		h+='<div class="table-view captions">';
		for(let i in mine){
			let p=mine[i];
...
function pm_dispute_create_form(market_id,ocs){
	...
	html+='<p><label class="input-descr"><span class="input-caption">'+(ltmp_arr.pm_dispute_proposed||'Proposed outcome')+':</span><select name="pm-dispute-oc" class="simple-rounded simple-rounded-size">'+opts+'</select></label></p>';
	html+='<p><label class="input-descr"><span class="input-caption">'+(ltmp_arr.pm_dispute_reason||'Reason / evidence')+':</span><textarea name="pm-dispute-reason" class="simple-rounded wide"></textarea></label></p>';
	html+='<p class="red pm-dispute-error"></p><p class="green pm-dispute-success"></p>';
	html+='<p><input class="pm-dispute-send blue-button captions" type="button" value="'+(ltmp_arr.pm_submit||'Submit')+'" data-market="'+market_id+'"><span class="submit-button-ring"></span><span class="icon icon-margin hidden icon-color-blue icon-check"></span></p>';
	f.html(html).css('display','block');
...
function fixed_award(account,amount,max_energy,memo,encode,el){
	let page=$(el).closest('.page');
	page.find('.fixed-award-action').attr('disabled','disabled');
	page.find('.icon-check').css('display','none');
	page.find('.submit-button-ring').css('display','inline-block');

	page.find('.fixed-award-error').html('');
	page.find('.fixed-award-success').html('');

	page.find('input[name=fixed-award-account]').val(account);
	page.find('input[name=fixed-award-account]').removeClass('red');
	if((''==account) || (!(/^([a-z0-9\-\.]*)$/).test(account))){
		page.find('input[name=fixed-award-account]').addClass('red');
		page.find('input[name=fixed-award-account]').focus();
		page.find('.fixed-award-error').html(ltmp_arr.default_recipient_error);

		page.find('.fixed-award-action').removeAttr('disabled');
		page.find('.submit-button-ring').css('display','none');
		return;
	}

	// reward_amount — фиксированная сумма в VIZ (TOKEN_SYMBOL, >0)
	amount=fast_str_replace(',','.',amount);
	let reward=parseFloat(amount);
	page.find('input[name=fixed-award-amount]').removeClass('red');
	if(isNaN(reward) || (reward<=0)){
		page.find('input[name=fixed-award-amount]').addClass('red');
		page.find('input[name=fixed-award-amount]').focus();
		page.find('.fixed-award-error').html(ltmp_arr.default_operation_error);

		page.find('.fixed-award-action').removeAttr('disabled');
		page.find('.submit-button-ring').css('display','none');
		return;
	}
	let reward_amount=reward.toFixed(3)+' VIZ';

	// max_energy — потолок энергии (%*100, 0..10000); пусто = 100%
	max_energy=fast_str_replace(',','.',max_energy);
	if(''==max_energy){max_energy='100';}
	let max_energy_int=Math.floor(parseFloat(max_energy).toFixed(2)*100);
	if(isNaN(max_energy_int) || (max_energy_int<0)){max_energy_int=0;}
	if(max_energy_int>10000){max_energy_int=10000;}
		...
		if(typeof response[0] !== 'undefined'){
			...
			if(encode){
				...
				if(''!=memo_key_input){
					try{
						...
						page.find('.fixed-award-error').html(ltmp_arr.default_incorrect_private_key_try_again);
						page.find('.fixed-award-action').removeAttr('disabled');
						page.find('.submit-button-ring').css('display','none');
						return;
					}
					...
					page.find('.fixed-award-error').html(ltmp_arr.default_type_memo_key);
					page.find('.fixed-award-action').removeAttr('disabled');
					page.find('.submit-button-ring').css('display','none');
					return;
				}
				if(false!==encoded_memo){
					...
					page.find('.fixed-award-error').html(ltmp_arr.default_memo_encode_error);
					page.find('.fixed-award-action').removeAttr('disabled');
					page.find('.submit-button-ring').css('display','none');
					return;
				}
			...
			viz.broadcast.fixedAward(users[current_user].active_key,current_user,account,reward_amount,max_energy_int,0,memo,beneficiaries_list,function(err,result){
				if(!err){
					...
					page.find('input[name=fixed-award-amount]').val('');
					page.find('input[name=fixed-award-memo]').val('');
					page.find('input[name=encode-memo]').prop('checked',false);
					page.find('input[name=memo-key]').val('');
					page.find('.memo-key-optional').css('display','none');

					update_balances($('.page-fixed-award .account-balance'));
...
window.onpopstate=function(event){
	...
	if(standalone){
		parse_standalone_fullpath();
		if(standalone_fullpath && standalone_fullpath.charAt(0)!=='/'){
			return;
		}
		change_state(standalone_path+standalone_search,state,false);
	}
	else{
		change_state(document.location.pathname+document.location.search,state,false);
	}
};
...
function app_mouse(e){
	if(!e)e=window.event;
	var target=e.target || e.srcElement;
	var normalized=$(target).closest('[data-href], .go-top, .select-lang-action, .menu-button-action, .add-account, .drop-down, .logout, .wallet-lock-btn, .select-user, .remove-user, [class$="-action"], [class*="-action "]');
	...
	*/
	...
	if($(target).hasClass('fixed-award-action')){
		let account=$('.page-fixed-award input[name=fixed-award-account]').val().toLowerCase().trim();
		let amount=$('.page-fixed-award input[name=fixed-award-amount]').val().trim();
		let max_energy=$('.page-fixed-award input[name=fixed-award-max-energy]').val().trim();
		let memo=$('.page-fixed-award input[name=fixed-award-memo]').val().trim();
		let encode=$('.page-fixed-award input[name=encode-memo]').prop('checked');
		fixed_award(account,amount,max_energy,memo,encode,target);
	}
...
function init_bindings(callback){
	...
	*/
	...
	if(window.MutationObserver){
		new MutationObserver(function(){ clearTimeout(window.app_a11y_timer); window.app_a11y_timer=setTimeout(app_a11y_enhance,50); }).observe(document.body,{childList:true,subtree:true});
	}
