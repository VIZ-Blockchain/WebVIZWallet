...
`,
				...
				<div class="account-keys hidden">
					...
					<div class="account-keys-master captions">
						...
						<div class="add-key-auths">
							...
							<input class="simple-inline" type="password" name="private-key" placeholder="私钥">
	...
	pm_pool_emergency_confirm:'现在紧急提取？罚金仅针对已累积奖励（绝不触及本金）。若资金池当前无空闲余额，提取将进入队列，随资金回流分批支付。',
	pm_pool_pending:'等待提取',
	pm_pool_queue_note:'若资金池当前无空闲余额，提取将进入队列，随市场资金回流分批支付——资金池绝不会支付超过其持有的额度。',
	pm_pool_emergency_warn:'解锁前紧急退出。罚金仅针对累积奖励，绝不触及本金。请输入要退出的份额（0 = 全部）。',
	pm_pool_queued_notice:'你有一笔排队中的提取',
	pm_pool_queued_tail:'将随资金回流自动（先进先出）支付。',
	pm_outcome:'结果',
	pm_payout:'赔付',
	pm_place_bet:'下注',
	...
	header_lock_wallet:'锁定钱包',
	header_add_account:'添加账户',
	header_switch_account:'切换账户',
	header_logout:'退出',
	index_social_capital_adaptive_caption:'资本 (viz):',
	index_balance_adaptive_caption:'余额 (viz):',
	index_frozen_markets_title:'预言机保险抵押',
	index_frozen_pool_title:'锁定在惰性流动性池中',
	index_frozen_liquidity_title:'锁定为市场流动性',
	index_frozen_bets_title:'锁定在未结算投注中',
	index_frozen_leverage_title:'锁定为杠杆保证金',
	...
	validator_props_captions:{
		...
		/* 预测市场 (HF14) */
		pm_oracle_registration_fee:'注册预言机的费用',
		pm_min_oracle_insurance:'预言机最低保险押金',
		pm_market_creation_fee:'创建预测市场的费用',
		pm_min_liquidity:'市场最低初始流动性',
		pm_max_outcomes:'每个市场的最大结果数',
		pm_max_market_duration:'市场最长持续时间（秒）',
		pm_max_oracle_fee_percent:'预言机最高费用（百分比）',
		pm_oracle_accept_window_sec:'预言机接受市场的时间窗口（秒）',
		pm_listing_min_coverage_percent:'市场上架的最低覆盖率（百分比）',
		pm_betting_min_coverage_percent:'保持下注开放的最低覆盖率（百分比）',
		pm_default_time_penalty_percent:'默认时间惩罚（百分比）',
		pm_max_time_penalty:'最大时间惩罚（原始单位）',
		pm_dispute_fee:'发起争议的费用',
		pm_dispute_grace_sec:'结算前的争议宽限期（秒）',
		pm_oracle_dispute_response_sec:'争议中预言机的响应窗口（秒）',
		pm_dispute_auto_close_sec:'争议自动关闭超时（秒）',
		pm_dispute_vote_period_sec:'争议投票期（秒）',
		pm_dispute_approve_min_percent:'批准争议的最低份额（百分比）',
		pm_oracle_penalty_percent:'预言机败诉的惩罚（百分比）',
		pm_no_contest_penalty_percent:'无争议/作废市场的惩罚（百分比）',
		pm_dispute_reward_multiplier:'争议奖励倍数（基点）',
		pm_batch_epoch_blocks:'批处理周期长度（区块）',
		pm_reveal_window_blocks:'commit-reveal 揭示窗口（区块）',
		pm_commit_no_reveal_penalty_percent:'承诺但未揭示的惩罚（百分比）',
		pm_min_batch_bet:'最低批量下注金额',
		pm_min_bet:'最低下注金额（VIZ）',
		pm_commit_reveal_enabled:'启用 commit-reveal 下注（true/false）',
		pm_processing_cap_per_block:'每个区块处理的最大 PM 操作数',
		pm_settle_rows_per_block:'每块结算行数',
		committee_votes_per_request:'每个请求的最大委员会投票数',
		committee_vote_min_vesting:'委员会投票的最低有效 vesting（VIZ）',
		pm_dispute_votes_per_market:'每个市场的最大争议投票数',
		pm_dispute_vote_min_vesting:'争议投票的最低有效 vesting（VIZ）',
		pm_lazy_pool_enabled:'启用惰性流动性池（true/false）',
		pm_lazy_alloc_percent:'每个市场的惰性池分配（百分比）',
		pm_lazy_max_total_alloc_percent:'最大惰性分配总额（百分比）',
		pm_lazy_lock_sec:'惰性分配锁定期（秒）',
		pm_lazy_recall_step_percent:'惰性回收步长（百分比）',
		pm_lazy_emergency_penalty_percent:'紧急惰性回收惩罚（百分比）',
		pm_lazy_min_liquidity_fee_percent:'惰性池补贴的最低市场流动性费用（百分比）',
		pm_leverage_enabled:'启用杠杆交易（true/false）',
		pm_leverage_fund_percent:'可用于杠杆贷款的自由余额份额（整数百分比）',
		pm_leverage_max_per_position_bp:'每个仓位的最大杠杆敞口（百分比）',
		pm_leverage_pool_profit_percent:'杠杆池利润份额（整数百分比）',
		pm_leverage_safety_margin_percent:'杠杆开仓安全边际（整数百分比）',
		pm_leverage_max_slippage_percent:'最大杠杆滑点（整数百分比）',
		pm_leverage_min_market_liquidity:'允许杠杆的最低市场流动性',
		pm_leverage_max_position_ratio_percent:'最大杠杆仓位比率（整数百分比）',
		pm_leverage_expiration_buffer_sec:'杠杆到期缓冲（秒）',
		pm_leverage_m_factor_percent:'杠杆保证金系数 M（整数百分比）',
		pm_leverage_funding_rate_ppm_per_day:'杠杆资金费率（每日 ppm，1000000 = 100%）',
		pm_conversion_profit_cost_percent:'仓位转换利润成本（整数百分比）',
		pm_closed_market_retention_sec:'已关闭市场数据保留（秒）',
	},
	...
	ms_op_delegate:'委托权益（vesting）',
	ms_op_award:'奖励（award，用能量）',
	ms_op_validator_vote:'验证人投票',
	ms_op_validator_proxy:'验证人投票代理',
	ms_op_reward_sharing:'验证人奖励分成',
	ms_op_committee_create:'委员会：创建申请',
	ms_op_committee_vote:'委员会：对申请投票',
	ms_op_pool_deposit:'流动性池：存入',
	ms_op_pool_withdraw:'流动性池：提取',
	ms_actor:'发起账户（多签）',
	ms_dg_delegatee:'委托给',
	ms_dg_shares:'权益（SHARES，0 = 取消委托）',
	ms_aw_receiver:'接收者',
	ms_aw_energy:'能量，%',
	ms_vv_validator:'验证人',
	ms_vv_approve:'投票',
	ms_vv_yes:'支持',
	ms_vv_no:'取消投票',
	ms_vp_proxy:'代理账户（留空 = 清除）',
	ms_rs_rate:'奖励分成，%',
	ms_cc_url:'URL（说明）',
	ms_cc_worker:'执行者（收款账户）',
	ms_cc_min:'最小金额，VIZ',
	ms_cc_max:'最大金额，VIZ',
	ms_cc_duration:'期限，天（5–30）',
	ms_cv_reqid:'申请 ID',
	ms_cv_percent:'投票，%（−100…100）',
	ms_pd_amount:'金额，VIZ',
	ms_pw_shares:'权益份额（0 = 全部）',
	ms_pw_emergency:'提取类型',
	ms_pw_planned:'计划提取',
